...
const YearlyPackaging = ({ fetchYearlyData }) => {
  const chartRef = useRef(null);
  const [chartData, setYearChart] = useState(null);

  useEffect(() => {
    fetchYearlyData().then((response) => {
      setYearChart(response.year_packaging_chart);
    });
  }, [fetchYearlyData]);
...
export default connect(mapStateToProps, mapDispatchToProps)(YearlyPackaging);
