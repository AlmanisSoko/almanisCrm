...
const YearlyTransport = ({ fetchYearlyData }) => {
  const chartRef = useRef(null);
  const [chartData, setYearChart] = useState(null);

  useEffect(() => {
    fetchYearlyData().then((response) => {
      setYearChart(response.year_transport_chart);
    });
  }, [fetchYearlyData]);
...
export default connect(mapStateToProps, mapDispatchToProps)(YearlyTransport);
