...
class KilosComponent extends React.Component {
    ...
    async fetchHomePage() {
        ...
        let monthdatalist = [];
        for (let i = 0; i < homedata.data.month_kilos_chart.length; i++) {
            monthdatalist.push({
                x: new Date(homedata.data.month_kilos_chart[i].date),
                y: homedata.data.month_kilos_chart[i].amt,
            });
        }

        let kilodatalist = [];
        for (let i = 0; i < homedata.data.year_kilos.length; i++) {
            kilodatalist.push({
                x: new Date(homedata.data.year_kilos[i].date),
                y: homedata.data.year_kilos[i].amt,
            });
        }
